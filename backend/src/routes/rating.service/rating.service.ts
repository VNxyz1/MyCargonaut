...
export class RatingService {
  ...
  async selectAllRatingsByUserId(userId: number): Promise<GetUserRatingsDto> {
    const ratingsAsDriver = await this.ratingRepository.find({
      where: { rated: Equal(userId), driver: true, complete: true },
    });
    const ratingsAsPassenger = await this.ratingRepository.find({
      where: { rated: Equal(userId), driver: false, complete: true },
    });
